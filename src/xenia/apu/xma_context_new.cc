...
bool XmaContextNew::Work() {
  ...
  while (remaining_subframe_blocks_in_output_buffer_ >=
    ...
    // input can leave the rest of the frame undelivered.
...
const uint32_t XmaContextNew::GetNextPacketReadOffset(
  ...
  while (next_packet_index < current_input_packet_count) {
    ...
    // No frame starts in this packet, so it is entirely the continuation of a
    // frame split across a packet boundary. Follow this packet's skip count to
    // keep walking the same sub-stream instead of merging into another one.
